                        ...
                        @foreach ($payments as $index => $payment)
                            <div class="d-flex gap-2 mb-2 align-items-center">
                                <select class="form-select form-select-sm fw-medium"
                                    wire:model.live="payments.{{ $index }}.mode_id" style="width:40%;">
                                    <option value="">Mode</option>
                                    @foreach ($paymentModes as $mode)
                                        <option value="{{ $mode->id }}">{{ $mode->name }}</option>
                                    @endforeach
                                </select>
                                <input type="number" class="form-control form-control-sm fw-bold text-end"
                                    wire:model.live.debounce.500ms="payments.{{ $index }}.amount" onclick="this.select()"
                                    placeholder="₹ Amount">
                                @if (count($payments) > 1)
                                    <button wire:click="removePaymentRow({{ $index }})" class="btn btn-sm text-danger p-0"><i
                                            class="feather-trash-2 fs-14"></i></button>
                                @endif
                            </div>
                        @endforeach
