                                                                        ...
                                                                        @if(!empty($lt['parameters']))
                                                                            <div class="mt-1 d-flex flex-wrap gap-1">
                                                                                @foreach($lt['parameters'] as $param)
                                                                                    <span class="badge bg-gray-200 text-dark fs-9">{{ is_array($param) ? ($param['name'] ?? json_encode($param)) : $param }}</span>
                                                                                @endforeach
                                                                            </div>
                                                                        @endif
                                                                    </div>
                                                ...
                                                <td colspan="5" class="bg-gray-50 ps-4 py-2 border-0">
                                                    ...
                                                    <div class="d-flex flex-wrap gap-1">
                                                        @foreach($item['parameters'] as $param)
                                                            <span class="badge bg-white border text-dark fs-10 px-2 py-1"><i class="feather-check-circle text-success me-1 fs-9"></i>{{ is_array($param) ? ($param['name'] ?? json_encode($param)) : $param }}</span>
                                                        @endforeach
                                                    </div>
                                                </td>
                                            </tr>
                    ...
                    <div class="card-body py-3">
                        ...
                        @elseif($selectedPatient)
                            <div class="d-flex justify-content-between align-items-center mb-2 p-2 rounded-3 border" style="background:rgba(255,193,7,0.1);border-color:rgba(255,193,7,0.3)!important;">
                                <span class="fw-bold fs-11" style="color:#cc9a06;"><i class="feather-award me-1 fs-10"></i>No Membership</span>
                                <button wire:click="$set('isMembershipModalOpen', true)" class="btn btn-sm btn-warning fw-bold fs-10 px-2 py-1"><i class="feather-plus fs-10 me-1"></i>Buy</button>
                            </div>
                        @endif
                                ...
                                <div class="col-md-4 col-sm-6">
                                    <div wire:click="$set('selectedMembershipId', {{ $mem->id }})" class="card h-100 border-2 {{ $selectedMembershipId == $mem->id ? 'border-primary shadow' : 'border-gray-200' }}" style="cursor:pointer;transition:all .2s;">
                                        <div class="card-body text-center p-3">
                                            ...
                                            <h6 class="fw-bold text-dark mb-1 fs-13">{{ $mem->name }}</h6>
                                            <div class="fs-2 fw-bold mb-1" style="color:{{ $mem->color_code ?? '#3b71ca' }};">₹{{ number_format($mem->price, 0) }}</div>
                                            <div class="badge bg-success bg-opacity-10 text-success fs-11 mb-1">{{ number_format($mem->discount_percentage, 0) }}% OFF</div>
                                            <div class="fs-11 text-muted">{{ $mem->validity_days }} days</div>
                                            @if($selectedMembershipId == $mem->id)<div class="mt-1"><i class="feather-check-circle text-primary fs-4"></i></div>@endif
                                        </div>
                                    </div>
                                </div>
                            @empty
                                <div class="col-12 text-center py-3 text-muted"><i class="feather-info fs-1 d-block mb-2 text-gray-300"></i><p>No plans configured.</p></div>
                            @endforelse
                        </div>
                    </div>
                    <div class="modal-footer">
