...
use App\Models\{Invoice, LabTest, DoctorProfile, PatientProfile, CollectionCenter, User, InvoiceItem, Payment, Department};
...
class Dashboard extends Component
{
    ...
    public function render()
    {
        ...
        $stats = [
            ...
            'total_doctors' => DoctorProfile::where('company_id', $companyId)->count(),
            'total_patients' => User::whereHas('patientProfile', fn($q) => $q->where('company_id', $companyId))->count(),
            'total_ccs' => CollectionCenter::where('company_id', $companyId)->count(),
        ];
        ...
        // 4. Rankings (Top 5)
        $topPackages = InvoiceItem::whereHas('invoice', fn($q) => $q->where('company_id', $companyId)->whereBetween('created_at', [$start, $end]))
        ...
        $topTests = InvoiceItem::whereHas('invoice', fn($q) => $q->where('company_id', $companyId)->whereBetween('created_at', [$start, $end]))
        ...
        $deptData = InvoiceItem::whereHas('invoice', fn($q) => $q->where('invoices.company_id', $companyId)->whereBetween('invoices.created_at', [$start, $end]))
        ...
        $paymentData = Payment::where('payments.company_id', $companyId)
            ...
            ->select('payment_modes.name as mode_name', DB::raw('SUM(amount) as total_collected'))
            ->groupBy('mode_name')
